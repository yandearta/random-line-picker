...
import { Box, Button, Container, Flex, Image, Text, Textarea, useToast, Wrap } from "@chakra-ui/react"
...
export const App = () => {
  ...
  const handleClick = () => {
    ...
    setResult((prevResult) => (prevResult === randomtext ? getRandomArray(splittedText) : randomtext))
  ...
  return (
    <Container maxW="75em" mt="4rem">
      ...
      <Textarea mt="2rem" minH="8rem" mb={3} placeholder="Paste your text here" value={text} onChange={(e) => setText(e.target.value)} />

      <Button colorScheme="messenger" leftIcon={<BsDice5 />} onClick={handleClick} disabled={isTextEmpty} mr={2}>
        Pick Random Line
      </Button>
      ...
      {result && (
        <Box mt="3rem">
          ...
          <Box p="1rem" maxW="100%" mb={3} display="inline-block" position="relative" borderRadius="md" style={{ border: "1.5px solid #e3e9f0" }}>
            <Text fontSize="3xl">{result}</Text>
            <Box id="confettiId" position="absolute" left="50%" top="55%" transform="translateX(-50%)" />
          </Box>

          <CopyToClipboard text={result} onCopy={handleOnCopy}>
            <Button display="flex" colorScheme="messenger" leftIcon={<FiCopy />}>
              Copy
            </Button>
          </CopyToClipboard>
        </Box>
